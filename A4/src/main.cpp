...
static void cursor_position_callback(GLFWwindow* window, double xmouse, double ymouse)
{
	...
	double ymax = links.size();
	double xmax = aspect*ymax;
	Vector2d x;
	x(0) = 2.0 * xmax * ((xmouse / width) - 0.5);
	x(1) = 2.0 * ymax * (((height - ymouse) / height) - 0.5);
	if(keyToggles[(unsigned)' ']) {
		cout <<"cursor: <" << x[0] << ", " << x(1) << ">" <<endl;
		target<<x(0), x(1); 

		VectorXd thetas(links.size());

		for (int i = 0; i < links.size(); ++i) {
			thetas[i] = links[i]->getAngle();
		}

		thetas = solver->optimize(thetas, target);
...
static void init()
{
	...
	int numLinks = NLINKS;
	links.push_back(std::make_shared<Link>(Link()));
